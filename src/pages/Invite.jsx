  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none fixed inset-0 mix-blend-screen opacity-20"
           style={{ backgroundImage: `linear-gradient(rgba(255,255,255,0.06), rgba(255,255,255,0.06)), repeating-linear-gradient(0deg, rgba(255,255,255,0.07) 0, rgba(255,255,255,0.07) 1px, transparent 1px, transparent 3px)` }} />

      <div className="max-w-2xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          <Link to="/" className="text-sky-300 hover:text-sky-200 text-sm">← Home</Link>
          ...
          <div className="hidden sm:flex items-center gap-2 px-3 py-1 rounded-full border border-white/10 bg-white/5 shadow-[0_4px_16px_rgba(0,0,0,.4)]">
            ...
            <span className="text-[11px] tracking-widest uppercase text-white/80">Blackwood Tower</span>
          </div>
        </div>
...
function Envelope({ phase, onPrimaryClick, onLetterClick, reduceMotion }) {
  ...
  return (
    <motion.div
      ...
      transition=
        {isSealed && !reduceMotion
          ? { duration: 1.4, repeat: Infinity, repeatDelay: 1.8, ease: "easeInOut" }
          : { duration: 0.3 }}
    >
      <div className="text-center mb-5 select-none">
        <div className="text-[11px] tracking-[0.25em] text-white/70 uppercase">You're invited</div>
        <div className="text-xs text-white/60">Tap {isSealed ? "the envelope" : "the letter"} to continue</div>
      </div>

      <div className="relative mx-auto w-[min(520px,92%)]" style={{ perspective: 1400 }}>
        ...
        >
          {/* Envelope base */}
          <div className="absolute inset-0 rounded-[22px] overflow-visible border border-amber-300/70 shadow-[0_20px_60px_rgba(0,0,0,.45)]"
               style={{
                 background: "linear-gradient(135deg,#f7e7b6 0%,#ead095 40%,#e6c37c 100%)",
               }}>
            {/* subtle inner edge */}
            ...
            <div className="absolute left-0 top-1/2 -translate-y-1/2 h-52 w-52 opacity-70"
                 style={{ clipPath: "polygon(0 0,100% 50%,0 100%)", background: "linear-gradient(135deg,#f5deaa,#e8c985)" }} />
            <div className="absolute right-0 top-1/2 -translate-y-1/2 h-52 w-52 opacity-70"
                 style={{ clipPath: "polygon(0 50%,100% 0,100% 100%)", background: "linear-gradient(225deg,#f5deaa,#e8c985)" }} />
            ...
            <motion.div
              ...
              className={`absolute left-1/2 top-7 -translate-x-1/2 w-[86%] rounded-xl bg-white border border-amber-300/70 z-20 ${
                isPeek ? "cursor-pointer" : ""
              }`}
              initial={false}
              animate={{
                y: isPeek ? -195 : 0, // slide upward out of the envelope
                height: isPeek ? 500 : 208,
                boxShadow: isPeek
              ...
              transition={{
                ...
                stiffness: 140,
                damping: 16,
                delay: isPeek && !reduceMotion ? 0.42 : 0.08, // follows flap
              }}
              style={{ overflow: isPeek ? "visible" : "hidden", zIndex: isPeek ? 50 : 20 }}
            >
              {/* sheen sweep */}
              <motion.div
                aria-hidden
                className="pointer-events-none absolute -inset-8 skew-x-[20deg] opacity-0"
                initial={false}
                animate={{
                  x: isPeek ? ["-40%", "120%"] : "-40%",
                  opacity: isPeek ? [0, 0.12, 0] : 0,
                }}
                transition={{ duration: 1.2, repeat: isPeek && !reduceMotion ? Infinity : 0, repeatDelay: 2.5 }}
                style={{
                  background:
                    "linear-gradient(90deg, transparent, rgba(255,255,255,.6) 45%, rgba(255,255,255,.9) 50%, rgba(255,255,255,.6) 55%, transparent)",
                }}
              />

              <motion.div className="h-full w-full p-5 text-neutral-800 relative" initial={false} animate={{ opacity: isPeek ? 1 : 0 }} transition={{ duration: 0.45, delay: isPeek && !reduceMotion ? 0.95 : 0 }}>
                {/* faux letterhead */}
                <div className="text-center">
                  <div className="text-[10px] tracking-[0.35em] text-neutral-500 uppercase">Blackwood Tower</div>
                  <div className="mt-1 text-[22px] font-serif">Yearly Company Dinner Party</div>
                </div>
                <div className="mt-4 text-sm leading-relaxed">
                  You are cordially invited to an evening of celebration, intrigue, and collaboration. Formal attire suggested. Dinner to follow.
                </div>
                <ul className="mt-4 mb-2 text-sm text-neutral-700 space-y-2">
                  <li>• Oct 4th at 5 pm</li>
                  <li>• Dress in character / Business Casual</li>
                  <li>• There will be food but it will not be dinner</li>
                  <li>• Backstories are important, take a second to read through yours</li>
                  <li>• This will be outside and includes walking</li>
                  <li>• <span className="font-bold text-red-600">COME WITH YOUR <span className="underline">PHONE FULLY CHARGED</span></span></li>
                </ul>
                <div className="mt-4 text-right text-xs text-neutral-500">— Blackwood Tower</div>
                <div className="mt-2 text-sm font-semibold text-sky-700 bg-sky-100/90 px-3 py-1 rounded-full inline-block select-none">
              ...
              {isPeek && (
                <motion.div
                  ...
                  transition={{ duration: 1.2, repeat: !reduceMotion ? Infinity : 0 }}
                />
              )}
            </motion.div>
            ...
            <div
              ...
              style={{
                ...
                background: "linear-gradient(180deg, rgba(250,234,177,0.95), rgba(236,205,130,0.96))",
            ...
            >
              <img src="/images/seal.png" alt="Wax seal" className="w-full h-full object-cover" />
            </motion.div>
              ...
              <div className="absolute left-1/2 top-[52%] -translate-x-1/2 z-40">
                {[...Array(8)].map((_, i) => (
                  <motion.span
                    ...
                    style={{ background: "radial-gradient(circle at 30% 30%, #ff9ab0, #b20a2c)" }}
                    initial={{ opacity: 0, x: 0, y: 0, scale: 0.8 }}
                    animate={{
                      opacity: [0, 1, 0],
                      x: (Math.cos((i / 8) * Math.PI * 2) * 80) + Math.random() * 10,
                      y: (Math.sin((i / 8) * Math.PI * 2) * 50) + Math.random() * 8,
                      scale: [0.8, 1.2, 0.6],
                    }}
            ...
            <motion.div
              className={`absolute left-0 right-0 top-0 h-1/2 origin-top ${isPeek ? "z-30" : "z-50"}`}
              style={{ transformStyle: "preserve-3d", clipPath: "polygon(0 0,100% 0,50% 100%)", background: "linear-gradient(#f6e3b1,#e4c882)" }}
              initial={false}
              animate={{ rotateX: isPeek ? -180 : 0 }}
              transition={{ type: "spring", stiffness: 160, damping: 18, delay: !isSealed ? 0.18 : 0 }}
            >
              {/* flap thickness */}
              <div className="absolute inset-x-0 bottom-0 h-[10px]"
                   style={{ background: "linear-gradient(180deg, rgba(0,0,0,0.25), rgba(0,0,0,0))" }} />
              {/* faux stamp */}
              <div className="absolute right-6 top-3 h-10 w-8 rounded-sm border border-amber-700/40"
                   style={{ background: "linear-gradient(180deg,#fff 0%,#f0e4c0 100%)" }}>
                <div className="absolute inset-0 opacity-70"
                     style={{ backgroundImage: "repeating-linear-gradient(135deg, transparent, transparent 6px, rgba(0,0,0,.06) 6px, rgba(0,0,0,.06) 12px)" }} />
              </div>
            </motion.div>
          </div>
        </div>
...
function WaxSeal() {
  return (
    <div className="relative w-full h-full">
      <div className="absolute inset-0" style={{ background: "radial-gradient(circle at 30% 30%, #ff8aa1, #b20a2c 65%)" }} />
      <div className="absolute inset-0 mix-blend-overlay opacity-30" style={{ backgroundImage: "radial-gradient(ellipse at 50% 30%, rgba(255,255,255,.8), transparent 40%)" }} />
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="text-sm font-black text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] tracking-widest">BT</span>
      </div>
      {/* ripples */}
      <motion.div
        ...
        <div className="flex items-center gap-4">
          <div className="relative h-20 w-20 rounded-2xl overflow-hidden ring-1 ring-white/15 bg-gradient-to-br from-sky-500/40 to-indigo-500/40 grid place-items-center">
            ...
            <motion.div
              ...
              style={{
                background: "linear-gradient(90deg, transparent, rgba(255,255,255,.25) 45%, rgba(255,255,255,.55) 50%, rgba(255,255,255,.25) 55%, transparent)",
              }}
            />
          </div>
          <div>
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-md p-5 shadow-[0_12px_30px_rgba(0,0,0,.45)]">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-base font-semibold bg-clip-text text-transparent bg-gradient-to-b from-white to-white/70">{title}</h2>
